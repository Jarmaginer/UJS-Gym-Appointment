...
public:
    ...
    std::vector<std::string> headers_check = {
      "Host: gym.ujs.edu.cn",
      "Content-Length : 56",
      "Accept: application/json, text/javascript, */*; q=0.01",
      "X-Requested-With: XMLHttpRequest",
...
std::string findAvailableTimes(const std::string& jsonString) {
    ...
    std::string Check() {
        headers_check.push_back((std::string)"Cookie: PHPSESSID=" + token);
        std::string URL = "http://gym.ujs.edu.cn/index.php/index/item/check.html";
        std::string response = POST(URL, "data_id=" + dateid + "&item_id=2&area_id=" + areaid + "&date_time=" + getDate(), headers_check);
        return response;
    }
